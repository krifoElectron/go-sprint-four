...
// Параметры:
//
// action int — число шагов при беге.
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	meanSpeedMetersPerSec := meanSpeed(action, duration) * kmhInMsec
	heighMeters := height / cmInM

	// ваш код здесь
	return ((walkingCaloriesWeightMultiplier*weight +
		((meanSpeedMetersPerSec*meanSpeedMetersPerSec)/heighMeters)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
